const StorageManager = {
  ...
  getProgress() {
    try {
      const data = localStorage.getItem('python-workshop-progress');
      if (!data) {
  ...
  _saveProgress(progress) {
    try {
      localStorage.setItem('python-workshop-progress', JSON.stringify(progress));
      return true;
    } catch (error) {
  ...
  resetProgress() {
    this._saveProgress(this._getDefaultProgress());
  }
};
